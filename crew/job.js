(function () {
  ...
  async function submitAction(payload, fromRetry = false) {
    ...
    const input = fromRetry ? { ...S.pending, expectedRevision: S.job.expectedRevision } : { ...payload, jobId, requestId: crypto.randomUUID(), expectedRevision: S.job.expectedRevision };
    ...
    try {
      ...
      message(data.alreadyApplied ? 'This action was already saved. The current job is shown.' : 'Saved to the job.');
    } catch (error) {
  ...
  async function selectPhotos(files) {
    ...
    for (const file of selected) {
      try {
        const photo = { id: crypto.randomUUID(), user: S.user.user, jobId, category, caption, dataUrl: await compressPhoto(file), state: 'ready', persisted: true };
  ...
  async function uploadPhotos(ids) {
    ...
    try {
      for (const id of ids) {
        ...
        try {
          await refreshJob();
          const data = await api('/api/field-jobs', { jobId, requestId: photo.id, expectedRevision: S.job.expectedRevision, action: 'photo', category: photo.category, caption: photo.caption, dataUrl: photo.dataUrl });
  ...
  main.addEventListener('submit', async event => {
    ...
    if (form.id === 'field-login') {
      ...
      try { S.user = await api('/api/hub-auth', { username: form.username.value, password: form.password.value }); await load(); } catch (error) { renderLogin(error.message); } finally { button.disabled = false; }
