...
test('public gallery uses the existing photographic before-after pair',()=>{
 ...
 assert.equal(new Set(pairs.map(p=>p.id)).size,7);
 assert.equal((html.match(/class="ba-card"/g)||[]).length,7);
 assert.equal((html.match(/class="after-image"/g)||[]).length,7);
 assert.equal((html.match(/class="before-image"/g)||[]).length,7);
