...
export async function onRequestGet({request,env}) {
  try {
    ...
    return reply(200,{ok:true,enabled:env.EGC_OPERATIONS_ENABLED==='true',actor:{id:session.user,role:session.role,kind:'human',workspace:env.EGC_OPERATIONS_WORKSPACE||'egc'},
...
export async function onRequestPost({request,env}) {
  ...
  try {
    ...
    if(!env.EGC_OPERATIONS_API_ORIGIN||!env.EGC_OPERATIONS_PORTAL_SIGNING_SECRET)return reply(503,{error:'operations_bridge_not_configured'});
    const origin=new URL(env.EGC_OPERATIONS_API_ORIGIN);
    if(origin.protocol!=='https:'||origin.username||origin.password||origin.pathname!=='/'||origin.search||origin.hash)return reply(503,{error:'operations_bridge_not_configured'});
    ...
    const envelope=await signOperationsEnvelope({v:1,iss:'portal',aud:'egc-operations',iat:Math.floor(Date.now()/1000),nonce:crypto.randomUUID(),actor,request:{requestId:input.requestId,body:input.body}},env.EGC_OPERATIONS_PORTAL_SIGNING_SECRET);
    const upstream=await fetch(new URL('/operations/rpc',origin),{method:'POST',redirect:'error',headers:{'Content-Type':'application/json'},body:JSON.stringify({envelope}),signal:AbortSignal.timeout(20000)});
