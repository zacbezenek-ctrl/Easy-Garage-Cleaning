...
const COLLECTIONS = new Set(['profiles', 'timeEntries', 'announcements', 'requests', 'incidents', 'equipment', 'training']);
...
function visibleTo(session, collection, data) {
  if (manager(session) || collection === 'announcements') return true;
...
async function authorizeMutation(env, session, collection, id, incoming, existing) {
  ...
  if (manager(session)) return { ...(existing || {}), ...incoming, id };
  ...
  if (collection === 'training') return { ...(existing || {}), ...incoming, id, employee: session.user };
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    const forbidden = /only|belongs/i.test(message);
    return reply(forbidden ? 403 : 502, { ok: false, error: message });
  }
}
