import { galleryPublicPairs as galleryPreviewPairs, galleryPublicVersion } from './_lib/gallery-public-data.js';
import { publicGalleryAsset } from './before-after-preview.js';

// Public inspiration-gallery presentation. No customer records or paid-generation calls.
// The original staff gallery, access controls and generation provenance remain unchanged.
export const galleryRelease = galleryPublicVersion;
...
export function renderPublicGallery() {
 const cards = galleryPreviewPairs.map((pair, index) => {
  ...
  return `<article class="card" data-search="${escape((pair.title+' '+pair.id+' '+(pair.keywords||'')).toLowerCase())}" data-scene="${escape(pair.id)}">
  ...
  <div class="comparison" style="--position:50%">
   <div class="image-stage"><img class="after-image" src="${after}" alt="AI-generated after concept: ${title}" width="${pair.width||1600}" height="${pair.height||1200}" loading="${index===0?'eager':'lazy'}" decoding="async"${index===0?' fetchpriority="high"':''}><img class="before-image" src="${before}" alt="AI-generated before concept: ${title}" width="${pair.width||1600}" height="${pair.height||1200}" loading="${index===0?'eager':'lazy'}" decoding="async"><span class="image-label label-before">Before</span><span class="image-label label-after">After</span><span class="divider" aria-hidden="true"><span>↔</span></span></div>
...
<title>Garage Before &amp; After Ideas | Easy Garage Cleaning</title><meta name="description" content="Explore 24 garage cleanout and organization concepts, from heavy clutter to black metal shelving and usable parking space. Plan your garage turnaround in Northern Colorado."><link rel="canonical" href="${galleryCanonical}">
...
<main class="shell"><section class="hero public-hero"><div><p class="eyebrow">Garage cleanouts + organization</p><h1>Before.<br><em>After.</em></h1></div><div class="hero-copy"><p class="intro">More room for what matters.</p><p>Heavy-duty black metal shelving. Organized bins. Space to park again. Explore ideas for a complete garage turnaround.</p><a class="gallery-cta" href="/book" data-cta="gallery-hero">Get my free garage plan ↗</a><p class="concept-note">AI-generated design concepts. Not completed customer projects.</p></div></section>
