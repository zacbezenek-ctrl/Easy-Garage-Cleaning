...
export type ExtractionResult = { records: SourceRecord[]; status: "complete" | "partial"; error: string | null; model: string | null };
...
export function validateExtractedEvent(value:unknown, source:SourceRecord): EvidenceEvent | null {
  ...
  return {eventType,confidence:row.confidence,supportingText:row.supportingText,humanReviewNeeded,
    nextAction:typeof row.nextAction==="string"?row.nextAction:null,details:{extractionMethod:"structured_semantic",customerCommitmentVerified:row.customerCommitmentVerified===true,...(["walkthrough_completed","job_completed"].includes(eventType)?{occurredAtVerified:false,timestampQuality:"retrospective_confirmation"}:{}),...(typeof row.timeMention==="string"?{timeMention:row.timeMention}:{}),...(typeof row.deadlineMention==="string"?{deadlineMention:row.deadlineMention}:{}),...(typeof row.reason==="string"?{reason:row.reason}:{})}};
}
...
export async function extractStructuredEvidence(records:SourceRecord[], context:SourceRecord[]=[], options:{useAI?:boolean;model?:string}={}):Promise<ExtractionResult> {
  ...
  if(!semantic.length)return {records:local,status:"complete",error:null,model:null};
  if(options.useAI===false || !process.env.OPENAI_API_KEY)return {records:local,status:"partial",error:options.useAI===false?"semantic_extraction_disabled":"semantic_extraction_unavailable",model:null};
  ...
  try {
    ...
    if(input.length>220_000)return {records:local,status:"partial",error:"semantic_input_requires_pagination",model};
    const response=await client.responses.create({model,instructions,input,
      text:{format:{type:"json_schema",name:"customer_sales_evidence",strict:true,schema:{type:"object",additionalProperties:false,required:["reviewedSourceIds","events"],properties:{reviewedSourceIds:{type:"array",items:{type:"string"}},events:{type:"array",items:{type:"object",additionalProperties:false,required:["sourceRecordId","eventType","supportingText","confidence","humanReviewNeeded","customerCommitmentVerified","nextAction","timeMention","deadlineMention","reason"],properties:{sourceRecordId:{type:"string"},eventType:{type:"string",enum:EVENT_TYPES.filter(t=>!["lead_created","walkthrough_booked","walkthrough_showed","job_scheduled","deposit_collected","revenue_collected"].includes(t))},supportingText:{type:"string"},confidence:{type:"number"},humanReviewNeeded:{type:"boolean"},customerCommitmentVerified:{type:"boolean"},nextAction:{type:["string","null"]},timeMention:{type:["string","null"]},deadlineMention:{type:["string","null"]},reason:{type:"string"}}}}}}}}});
    ...
    let invalid=semantic.filter(s=>!reviewed.has(s.sourceRecordId)).length;
    for(const target of local)if(semantic.some(s=>s.sourceRecordId===target.sourceRecordId)&&reviewed.has(target.sourceRecordId))target.events=target.events.filter(e=>safeLocal.has(e.eventType));
    for(const proposed of parsed.events){const id=asRecord(proposed).sourceRecordId;const target=local.find(r=>r.sourceRecordId===id);if(!target){invalid++;continue;}
      const event=validateExtractedEvent(proposed,target);if(!event){invalid++;continue;}
      const existing=target.events.findIndex(e=>e.eventType===event.eventType);
      ...
      if(verifiedLocal&&!event.humanReviewNeeded){const persisted=target.events.find(e=>e.eventType===event.eventType)!;persisted.valueCents=verifiedLocal.valueCents!;persisted.valueVerified=true;persisted.currency=verifiedLocal.currency!;persisted.details={...persisted.details,...verifiedLocal.details};}
    }
    return {records:local,status:invalid?"partial":"complete",error:invalid?`unsupported_or_unquoted_events:${invalid}`:null,model};
  }catch(error){
    // Never persist provider bodies or authentication errors containing secrets.
    const code=error instanceof OpenAI.APIError ? `semantic_provider_http_${error.status ?? "unknown"}` : "semantic_extraction_failed";
    return {records:local,status:"partial",error:code,model};
  }
}
