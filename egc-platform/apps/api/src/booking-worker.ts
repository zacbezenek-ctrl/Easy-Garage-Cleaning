...
export async function reconcileHubBookings(portal:Portal,env:NodeJS.ProcessEnv=process.env){
 ...
 for(let page=0;page<30;page++){
  ...
  for(const raw of result.items){
   ...
   const detail=await portal(actor,{command:'portal.job',jobId:visit.id}),job=rec(detail.job);
  ...
  if(typeof result.nextOffset!=='number'||result.nextOffset<=offset)throw new Error('hub_calendar_pagination_stalled');
 ...
 if(requested.length){
  ...
  }catch{evidenceError='hub_evidence_unavailable_or_invalid';}
 }
 ...
 const evidenceDiagnostics={requestedContacts:requested.length,reconciledContacts:selected.length,selectionComplete,complete:evidenceComplete,asOf:evidenceAsOf,error:evidenceError,unmappedCalendarContacts:calendarContactIds.filter(id=>!contactRows.some(c=>c.providerId===id))};
 const diagnosticResult={...result,portalEvidence:evidenceDiagnostics};
 await db.insert(schema.syncCursors).values({key:'customer_state:booking_reconciliation',cursor:JSON.stringify(diagnosticResult)}).onConflictDoUpdate({target:schema.syncCursors.key,set:{cursor:JSON.stringify(diagnosticResult),updatedAt:new Date()}});
