...
export class InboundActionReconciler{
  ...
  async run(options:{lookbackDays?:number;limit?:number}={}){
    ...
    return this.db.transaction(async tx=>{
      ...
      const status={lastAttemptAt:now.toISOString(),activationAt:activation!.cursor,windowStart:from.toISOString(),checked:0,created:0,alreadyPresent:0,completed:0,failed:0,blocked:0,hasMore:false,ownerSource:'unknown',dueMinutes:null as number|null,errorCode:null as string|null,lastSuccessAt};
      try{
        ...
        if(!policy.inboundResponse?.enabled){status.blocked=status.checked;status.errorCode=policy.inboundResponse?.blockedReason||'inbound_policy_unresolved';await this.save(status);return{ok:false,...status};}
        ...
        for(const raw of answered){const r=raw as unknown as {id:string;revision:number;responseId:string};try{await this.service.execute(this.actor,{command:'task.complete',taskId:r.id,revision:r.revision,outcome:'Verified later human outbound message: '+r.responseId},inboundRequestId('completed:'+r.id+':'+r.responseId));status.completed++;}catch{status.failed++;}}
        if(status.failed)status.errorCode='inbound_action_write_failed';else status.lastSuccessAt=this.clock().toISOString();await this.save(status);return{ok:status.failed===0,...status};
      }catch{status.errorCode='inbound_reconciliation_unavailable';status.failed++;await this.save(status);return{ok:false,...status};}
    });
  }
}
