...
import {OperationsError,operationsService,signRequest,verifyRequest,type Actor,type Command,type OperationsService,type PortalJobReference} from "@egc/operations";
...
export function portalAdapter(origin:string,key:string,workspace:string,fetcher:typeof fetch=fetch) {
  ...
  async function read(actor:Actor,body:Command) {
    ...
    if(!response.ok)throw new OperationsError("portal_authority_unavailable",503,{upstreamStatus:response.status});
    return result;
  }
...
export async function registerOperationsRoutes(app:FastifyInstance,options:{service?:OperationsService;env?:NodeJS.ProcessEnv}={}) {
  ...
  if(!service && env.EGC_OPERATIONS_ENABLED==="true") {
    ...
    service=operationsService({workspace,...(bridge?{resolvePortalJob:bridge.resolve,resolveOwner:bridge.owner,portalRead:bridge.read}:{})});
  }
