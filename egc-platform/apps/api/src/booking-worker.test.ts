...
import {reconciliationDiagnostic} from './reconciliation-diagnostics.js';
...
describe('Hub booking worker source boundary',()=>{
 ...
 it('refuses a provider calendar substituted for the Hub and propagates safe unavailable detail before writes',async()=>{await expect(reconcileHubBookings(vi.fn(async()=>({authority:'ghl',items:[],coverage:{complete:true}})),{EGC_BOOKING_AUTO_RECONCILE:'true'})).rejects.toThrow('hub_calendar_unavailable');const p=vi.fn(async(_a:Actor,c:Command)=>{if(c.command==='calendar')return {authority:'employee_hub',items:[visit()],coverage:{complete:true}};throw new Error('private portal token');});await expect(reconcileHubBookings(p,{EGC_BOOKING_AUTO_RECONCILE:'true'})).rejects.toMatchObject({message:'reconciliation_unavailable',diagnostic:{stage:'hub_job',errorCode:'reconciliation_unavailable'}});expect(f.syncPortalSchedule).not.toHaveBeenCalled();expect(f.writes).toEqual([]);});
 it('identifies calendar authentication failure and canonical ingestion failures without source text',async()=>{
  const rejected=vi.fn(async()=>{throw {code:'service_key_source_unavailable',status:503,details:{upstreamStatus:503},message:'secret'};});
  await expect(reconcileHubBookings(rejected,{})).rejects.toMatchObject({diagnostic:{stage:'hub_calendar',errorCode:'service_key_source_unavailable',httpStatus:503,upstreamStatus:503}});
  f.reconcileCustomerState.mockRejectedValueOnce(new Error('SQL customer body',{cause:{code:'42703'}}));
  const failure=await reconcileHubBookings(portal(),{}).catch(e=>e);expect(reconciliationDiagnostic(failure)).toEqual({stage:'canonical_ingestion',errorCode:'database_unavailable',sqlState:'42703'});expect(JSON.stringify(failure)).not.toContain('customer body');
 });
