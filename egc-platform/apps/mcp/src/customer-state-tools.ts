...
export async function canonicalOperationalReport(input:{days?:number|undefined;from?:string|undefined;to?:string|undefined;cohortFrom?:string|undefined;cohortTo?:string|undefined;refresh?:boolean|undefined;evidenceOffset?:number|undefined;evidenceLimit?:number|undefined}={}){
 ...
 const report=await getCanonicalReport({...window,...(input.cohortFrom?{cohortSince:input.cohortFrom}:{}),...(input.cohortTo?{cohortUntil:input.cohortTo}:{}),evidenceOffset:offset.parse(input.evidenceOffset??0),evidenceLimit:limit.parse(input.evidenceLimit??100),refresh:input.refresh??false});
...
export async function canonicalFunnel(days:number){
 const report=await canonicalOperationalReport({days,refresh:false});
...
export function registerCustomerStateTools(server:McpServer){
 server.registerTool('egc.operational_report',{description:'Read the stored canonical EGC operational snapshot for Morning Command, EOD, weekly and monthly reporting. This read never reconciles, extracts, enqueues, or persists customer state. Separates event-period activity from lead-cohort metrics with numerator, denominator, maturity and evidence for every conversion. Distinguishes walkthrough, video quote and direct-job pipelines, user-confirmed truth, provider lag and coverage failures. Evidence is paginated; retrieve every remaining source using egc.operational_event_evidence and the returned nextOffset. Use egc.reconcile_customer_state explicitly when reconstruction is authorized.',inputSchema:z.object({days:z.number().int().min(1).max(365).default(7),from:iso.optional(),to:iso.optional(),cohortFrom:iso.optional(),cohortTo:iso.optional(),evidenceOffset:offset.default(0),evidenceLimit:limit.default(100)}).strict(),...read},async args=>result(formatOperationalBriefing(await canonicalOperationalReport(args))));
