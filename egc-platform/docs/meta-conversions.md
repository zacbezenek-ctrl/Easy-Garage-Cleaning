...
Events Manager subsequently confirmed that dataset 1262944809378035 is a Conversions API for CRM integration connected to EGC's page, with no websites and three active CONVERTED events. Its current UI says the sales funnel is no longer available because CONVERTED has already been selected in the ads campaign; event changes are made in the campaign. The inspected history before this rollout contained no WALKTHROUGH_BOOKED or JOB_WON events. The source and meaning of the prior CONVERTED events remain unverified. This release collects two distinct custom CRM stages for future legitimate conversions; it neither redefines CONVERTED nor changes the campaign's selected event. Receipt of these new events does not establish that the current QUALITY_LEAD campaign optimizes toward them. An unavailable legacy mapping screen does not block this authorized stage collection; any later optimization-target or CONVERTED-definition change requires a separate decision.

Railway worker and MCP deployed commit 8452cc7fd2d9b9683606022cf2d6ad23582e0511 successfully in shadow mode on September 20. Migrations completed, API/portal/MCP health endpoints returned 200, and the authenticated MCP startup check exercised all five new tool registrations, preview, dry-run, status, and the existing lead funnel. Live preview found six eligible events and the ledger held six pending plus one skipped; zero were transmitted. The 30-day eligible Meta cohort had 42 leads and six walkthrough bookings (14.3%). These are observed cohort counts, not Meta-reported attribution or claimed incremental results.

The operator supplied META_CAPI_ACCESS_TOKEN directly in Railway for both sender services, and their credential deployments succeeded on September 20. The non-secret Test Events code was obtained from this dataset and configured server-side. Tokens stay only on Railway's egc-worker and egc-mcp services. Leave shadow mode enabled until both synthetic stages are accepted and visible.
...
- Explicit boolean provider test markers (`isTest`, `is_test`, `isTestLead`, `is_test_lead`) on raw contact or attribution data exclude the record with `explicit_test_record`. Names, email patterns and numeric lead IDs are not guessed to be test records. A test lead alone never creates a downstream event. Selecting the CRM Test Events channel can automatically generate a verification lead; avoid that channel for the synthetic CAPI test and never book or mark that verification lead won.
...
# Legacy variable name: verifies stage collection/test visibility, not optimization mapping.
...
4. Review collection of both exact custom names in the verified CRM dataset, inspect existing history for a sender already using either name, and visually confirm both synthetic events from step 3. Only then set `META_CAPI_FUNNEL_VERIFIED=true`. The environment name is retained for deployment compatibility; it attests this stage-collection review and visual test receipt. Status reports `stageCollectionVerified` and explicitly leaves optimization mapping unverified. This gate does not require the prior CONVERTED sender's meaning or an unavailable legacy funnel mapping to be resolved before collecting future custom events. Leave CONVERTED and the campaign target unchanged; mapping these custom events to the selected optimization signal is a separate decision.
...
If an already-connected MCP client has cached the older tool catalog, the operator can temporarily set `META_CAPI_TEST_ON_START=true` alongside that verification flag on MCP. In explicit shadow mode only, this invokes the same authenticated `meta.conversions.test` tool once during that startup and logs an allowlisted result. It never enables production syncing or accepts credentials from tool arguments. Reset the test flag to false after the attempt; leaving it enabled can repeat synthetic tests on later shadow-mode restarts. Both events must still be confirmed in Events Manager.
