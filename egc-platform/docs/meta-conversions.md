...
Events Manager subsequently confirmed that dataset 1262944809378035 is a Conversions API for CRM integration connected to EGC's page, with no websites and three active CONVERTED events. Its current UI says the sales funnel is no longer available because CONVERTED has already been selected in the ads campaign; event changes are made in the campaign. Do not assume a legacy funnel-mapping screen is available or that adding these custom names automatically changes the selected optimization signal. Before production, verify the source and meaning of the existing CONVERTED events and whether the new stages can feed the intended signal without changing the live ad-set target. Keep META_CAPI_FUNNEL_VERIFIED=false until this is resolved.
...
No META_CAPI_ACCESS_TOKEN exists in either sender service. The non-secret Test Events code was obtained from this dataset and configured server-side for the next deployment. Obtain a dataset-authorized CAPI token through Meta and supply it only as META_CAPI_ACCESS_TOKEN on Railway's egc-worker and egc-mcp services. Leave shadow mode enabled until the synthetic test and integration checks pass.
...
4. Verify the current CRM event selection behavior for both exact custom names and the prior CONVERTED sender. The current UI no longer offers the legacy sales funnel after a campaign event is selected. If feeding the selected signal requires changing the campaign target or redefining CONVERTED, stop for that consequential decision. Mark `META_CAPI_FUNNEL_VERIFIED=true` only after compatibility is verified. This gate also attests visual test confirmation.
