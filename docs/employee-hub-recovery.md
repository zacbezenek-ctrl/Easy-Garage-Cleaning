...
To add a missing staff account without exporting or replacing the existing sealed `HUB_AUTH_USERS_JSON`, set the optional encrypted secret `HUB_AUTH_ADDITIONAL_USERS_JSON` to a JSON object with the same username-to-record format. For example, a new `TylerG` entry can use `{"passwordHash":"<generated-pbkdf2-hash>","displayName":"Tyler","role":"manager"}`. Generate its hash with `createHubCredentialHash` using a password of at least 12 characters. Preserve any existing entries in the supplemental map as well, then redeploy Pages. Both maps are append-only relative to each other: any duplicate username after trimming and lowercasing, within either map or across maps, blocks staff authentication rather than replacing an account. Malformed configuration also fails closed. This setting does not approve pending employee applications, unlock the employee vault, or give anyone except `ZacB` account-approval permissions.
