...
function initMultiStepForm(form){{
  ...
  const names=['What do you need?','Where are you located?','Upload photos','Contact & timing'];
  ...
  const show=(n)=>{{panels.forEach((p,i)=>p.classList.toggle('active',i+1===n));dots.forEach((d,i)=>{{d.classList.toggle('active',i+1===n);d.classList.toggle('done',i+1<n);}});step=n;const lbl=form.querySelector('.form-step-label');if(lbl)lbl.textContent='Step '+n+' of '+total+(names[n-1]?': '+names[n-1]:'');if(pctEl)pctEl.textContent=Math.round((n/total)*100)+'%';focusStep(n);}};
  ...
  function validateStep(n){{
    ...
    if(n===2){{const city=form.querySelector('[name="City"]');if(city&&!city.value){{showErr(panel,'Please select your city so we can confirm service area.');city.focus();return false;}}}}
    if(n===4){{const name=form.querySelector('[name="Name"]');const phone=form.querySelector('[name="Phone"]');if(name&&!name.value.trim()){{showErr(panel,'Please enter your name.');name.focus();return false;}}if(phone&&!phone.value.trim()){{showErr(panel,'Please enter a phone number so we can call with your quote.');phone.focus();return false;}}}}
    return true;
  }}
    ...
    <div class="cta-layout reveal">
      ...
      <div class="quote-form">
        ...
        <div class="form-steps-wrap">
          <div class="form-progress-row"><span class="form-step-label">Step 1 of 4: What do you need?</span><span class="form-progress-pct" data-progress-pct>25%</span></div>
...
<a href="/hot-tub-removal-fort-collins-co.html">Hot tub removal</a><a href="/pricing.html">Pricing</a>
    ...
    text = re.sub(
        ...
        '<p class="pricing-disclaimer reveal">All quotes are flat-rate and include labor, hauling, dump fees, and donation drop-offs. <a href="/pricing.html" style="color:var(--accent);font-weight:600;">Full pricing guide →</a> · <a href="#quote" style="color:var(--accent);font-weight:600;">Get Free Quote →</a></p>',
        text,
        count=1,
    )
    for old, new in [
