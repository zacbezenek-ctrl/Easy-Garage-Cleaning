...
(async()=>{
 ...
 try{
  for(const width of [1440,390,320]){
   ...
   await page.locator('#search').fill('bikes');assert.equal(await page.locator('#gallery .card:visible').count(),1);
   await page.locator('#search').fill('nothing-matches-zz');assert(await page.locator('#empty').isVisible());
   await page.locator('#search').fill('');assert.equal(await page.locator('#gallery .card:visible').count(),6);
  ...
  await nojs.goto(url);assert.equal(await nojs.locator('noscript a').count(),12);
  assert.equal(await nojs.locator('#gallery .card').count(),6);await nojs.close();
