#!/usr/bin/env python3
"""Read-only verification of the exact public gallery release and its assets."""
...
expected=(ROOT/'before-after.html').read_bytes()

def get(path):
    request=urllib.request.Request(BASE+path,headers={'User-Agent':'EGC-Gallery-Release-Check/1.0','Cache-Control':'no-cache'})
...
for attempt in range(30):
    try:
        ...
        assert html==expected,'Public HTML is not yet the tested release'
        assert manifest['release'].encode() in html
